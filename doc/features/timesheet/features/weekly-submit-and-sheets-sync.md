...
| API policy | Server enforces leave (OVERRIDE ack), holiday ack, future ack, day total > 24 ack, hours > 0 for non-empty days; holiday/leave dependency unavailable → 503 (fail closed; never treat miss as empty holidays; never treat unrecognized/partial Zoho holiday payloads as empty) |
