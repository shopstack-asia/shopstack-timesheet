...
4. On failure → 500 asking admin to refresh cache.
...
- Holidays GET: session required; invalid year → 400.
- Cron: `Authorization` must equal `Bearer ${CRON_SECRET}`.
...
- Read path never fetches Zoho directly — stale/empty until refresh succeeds.
- `vercel.json` does not schedule this route.
