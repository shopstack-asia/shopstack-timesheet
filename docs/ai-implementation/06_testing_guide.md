...
| `confirmation.test.ts` | concurrent atomic claim, wrong user, expired, crash reclaim, Slack `event_id` prefer/dedupe |
...
| `partial-write.test.ts` | upsert failure must not call delete; delete failure full restore; compound restore failure |
